/**
 * Parts of this are based on meth-meth-method's code https://github.com/meth-meth-method/tetris/blob/master/tetris.js
 *
 * = TODO =
...
export class Tetris {
  ...
  */
		...
		this.currentShape.matrix.forEach((row, y) => {
			row.forEach((val, x) => {
				if (val !== 0) {
					...
					this.ctx.strokeRect(
						x + this.currentShapeOffset.x,
						y + this.currentShapeOffset.y,
						1,
						1
					);
	...
	checkGameOver() {
		if (
			this.boardMatrix[1][0] !== 0 ||
			this.boardMatrix[1][1] !== 0 ||
			this.boardMatrix[1][2] !== 0
		) {
			this.gameOver = true;
		}
	}
	...
	dropAtIntervals(t = 0) {
		...
		game = requestAnimationFrame((t) => {
			this.dropAtIntervals(t);
		});
	}
	...
	isCollission() {
		...
		for (let y = 0; y < m.length; ++y) {
			for (let x = 0; x < m[y].length; ++x) {
				if (
					m[y][x] !== 0 &&
					(this.boardMatrix[y + o.y] && this.boardMatrix[y + o.y][x + o.x]) !==
						0
				) {
					return true;
				}
	...
	addShapeToBoardMatrix() {
		this.currentShape.matrix.forEach((row, y) => {
			row.forEach((val, x) => {
				if (val !== 0) {
					this.boardMatrix[y + this.currentShapeOffset.y][
						x + this.currentShapeOffset.x
					] = val;
				}
			});
	...
	checkIfBottomRowIsInvisible() {
		...
		let copyLastRow = JSON.parse(
			JSON.stringify(this.currentShape.matrix[length - 1])
		);

		if (
			JSON.stringify(copyLastRow) ===
			JSON.stringify(copyLastRow.fill(0, 0, length))
		) {
			return true;
		}
		return false;
